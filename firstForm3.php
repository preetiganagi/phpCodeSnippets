...
require("header.html");
    ...
    $conn = mysqli_connect("localhost","root","compass","userdatabase");
    // Check connection
    if($conn === false){
...
<form method="post" action="">  
    ...
    <br> <br>
	  <b>Hobbies :</b><select name="hobby[]" id="hobbies" multiple="multiple" size=3>
	                   ...
	                   <?php while ($row = mysqli_fetch_assoc($hobbyResult)) { ?>					
		                  <option name="hobby" value="<?php echo $row["hobbyName"]; ?>"
		                      <?php if(!empty($_POST['hobby']))
                               {
			                        $hobbyarray = $_POST['hobby'];	
			
			                        if(in_array($row["hobbyName"], $hobbyarray))
			                        {
		...
		<b>Tech Languages:</b><br>
            <?php while ($row = mysqli_fetch_assoc($subjectResult)) { ?>
		        <input type="checkbox" name="subject[]" id ="subject" <?php echo $selected[$row["techName"]]; ?>  value="<?php echo $row["techName"]; ?>"> <?php echo $row["techName"] ?>&nbsp;
		    <?php } ?><br><br>
